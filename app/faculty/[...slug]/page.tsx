...
async function getFaculty(path: string): Promise<DrupalFaculty | null> {
  try {
    ...
    const data = await client.raw(GET_FACULTY_BY_PATH, { path })
...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  ...
  const item = await getFaculty(path)

  if (!item) {
    return { title: 'Faculty Not Found | Community College' }
  }
                ...
                <dl className="space-y-4">
                  {(item as any).positionTitle && (
                    <div>
                      ...
                      <dd className="font-semibold text-gray-900">{(item as any).positionTitle}</dd>
                    </div>
                  )}
                  {(item as any).email && (
                    <div>
                  ...
                  {(item as any).officeLocation && (
                    <div>
                      <dt className="text-sm text-gray-500">Office Location</dt>
                      <dd className="font-semibold text-gray-900">{(item as any).officeLocation}</dd>
                    </div>
                  )}
                  {(item as any).officeHours && (
                    <div>
                      <dt className="text-sm text-gray-500">Office Hours</dt>
                      <dd className="font-semibold text-gray-900">{(item as any).officeHours}</dd>
                    </div>
                  )}
                </dl>
                <div className="mt-8">
