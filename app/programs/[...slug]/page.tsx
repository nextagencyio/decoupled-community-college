...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  ...
  if (!item) {
    return { title: 'Program Not Found | Community College' }
  }

  return {
    title: `${item.title} | Community College`,
            ...
            <div className="lg:col-span-1">
              <div className="bg-white rounded-xl shadow-sm p-6 sticky top-24">
                ...
                <dl className="space-y-4">
                  {(item as any).credential && (
                    <div>
                      <dt className="text-sm text-gray-500">Credential Earned</dt>
                      <dd className="font-semibold text-gray-900">{(item as any).credential}</dd>
                    </div>
                  )}
                  ...
                  {(item as any).credits && (
                    <div>
                      ...
                      <dd className="font-semibold text-gray-900">{(item as any).credits}</dd>
                    </div>
                  )}
                  {(item as any).tuitionEstimate && (
                    <div>
                      <dt className="text-sm text-gray-500">Estimated Tuition</dt>
                      <dd className="font-semibold text-gray-900">{(item as any).tuitionEstimate}</dd>
                    </div>
                  )}
                  {(item as any).startDates && (
                    <div>
                      <dt className="text-sm text-gray-500">Start Dates</dt>
                      <dd className="font-semibold text-gray-900">{(item as any).startDates}</dd>
                    </div>
                  )}
                </dl>
                <div className="mt-8">
                  <Link
                    href="/contact"
                    className="block w-full text-center px-6 py-3 bg-primary-700 text-white rounded-lg font-bold hover:bg-primary-700 transition-colors"
                  >
                    Contact Us
                  </Link>
                </div>
